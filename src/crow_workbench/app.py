    ...
    *,
    ...
    def evidence_resolution_verification_file(project_id: str) -> Path:
        ...
        return (
            ...
            / _safe_project_id(project_id)
            / "building-graph"
            / "evidence-resolution-verifications.json"
        )

    def load_evidence_resolution_verifications(project_id: str) -> list[dict[str, Any]]:
    ...
    def get_rc1_workbench(project_id: str) -> dict[str, Any]:
        graph = building_graph_repository(project_id).load()
        graph_audits = load_persisted_audits(graph_audit_directory(project_id), graph_audit_pattern)
        evidence_audits = load_persisted_audits(
            evidence_audit_directory(project_id), "evidence-audit-*.json"
        )
        capabilities = {
            ...
            "rules": {"summary": {"registered": len(domain_graph_rules) + 4}},
    ...
    def list_evidence_resolution_verifications(
        ...
        items = load_evidence_resolution_verifications(project_id)
    ...
    @app.post(
        "/api/projects/{project_id}/graph/evidence-audit-runs/{base_audit_id}"
        "/compare/{target_audit_id}/findings/{finding_id}/verify",
        status_code=201,
    )
    def verify_evidence_audit_resolution(
        project_id: str,
        base_audit_id: str,
        target_audit_id: str,
        finding_id: str,
    ...
    def get_rule_explorer(project_id: str) -> dict[str, Any]:
        try:
            ...
            rules = []
            for rule in (*domain_graph_rules, *evidence_rules):
    ...
    def verify_audit_resolution(
        ...
        if any(
            ...
            raise HTTPException(status_code=409, detail="Upplösningskandidaten är redan verifierad")
        try:
            verification = GraphResolutionVerificationService(namespace=graph_audit_prefix).decide(
                base_audit_id=base_audit_id,
                target_audit_id=target_audit_id,
                finding_id=finding_id,
                lifecycle=change.lifecycle,
