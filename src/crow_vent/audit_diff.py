...
@dataclass(frozen=True)
class VentAuditDiffResult:
    base_audit_id: str
    target_audit_id: str
    changes: tuple[VentAuditFindingChange, ...]
    summary: dict[str, int]
    metadata: dict[str, Any] = field(default_factory=dict)


class VentAuditDiffer:
    """Compare immutable audit runs without mutating findings or graph state."""
    ...
    def compare(
        ...
        *,
        ...
        changes: list[VentAuditFindingChange] = []
        all_ids = sorted(set(base_findings) | set(target_findings))
