...
def test_does_not_guess_unlabelled_values() -> None:
    observation = make_observation("o2", "B1 FTX01 34 l/s 40 l/s")
    result = import_observations(
        ObservationCollection(project_id="p1", observations=(observation,))
    )
...
def test_imports_explicit_finding_conservatively() -> None:
    observation = make_observation("o3", "Anmärkning: Filter i FTX02 är smutsigt")
    result = import_observations(
        ObservationCollection(project_id="p1", observations=(observation,))
    )
...
def test_unrelated_observation_is_kept_for_review() -> None:
    observation = make_observation("o4", "Besiktning utförd 2025-04-02")
    result = import_observations(
        ObservationCollection(project_id="p1", observations=(observation,))
    )
