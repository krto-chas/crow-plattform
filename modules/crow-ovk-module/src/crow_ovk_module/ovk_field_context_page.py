...
_SPACE_DIGEST_RUNTIME = (
    ...
    "$('nameplateStatus').textContent='Foto kräver HTTPS. Övrig rondering kan testas över HTTP.';"
...
_PHOTO_DIGEST_RUNTIME = (
    ...
    "$('photoStatus').textContent='Foto kräver HTTPS. Övrig rondering kan testas över HTTP.';"
    "$('photo').value='';return}"
    "const buffer=await file.arrayBuffer();"
    "const hash=await globalThis.crypto.subtle.digest('SHA-256',buffer);"
)
