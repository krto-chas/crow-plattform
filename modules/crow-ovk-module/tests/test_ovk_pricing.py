...
def test_flerbostadshus_prices_per_apartment() -> None:
    ...
    quote = build_quote(
        _single(
            OvkObjectPart(
                ...
                apartment_count=33,
                system_type=VentilationSystemType.F,
            ),
            InspectionType.ATERKOMMANDE,
        ),
        taxa,
    )
    line = quote.lines[0]
    ...
    assert line.amount == Decimal(33) * taxa.rate(
        BuildingCategory.FLERBOSTADSHUS, InspectionType.ATERKOMMANDE
    )
...
def test_flerbostadshus_requires_apartment_count() -> None:
    ...
    with pytest.raises(ValueError, match="apartment_count is required"):
        build_quote(
            _single(
                OvkObjectPart(category=BuildingCategory.FLERBOSTADSHUS),
                InspectionType.ATERKOMMANDE,
            ),
            taxa,
        )
...
def test_smahus_flat_price_ignores_optional_size() -> None:
    taxa = load_taxa()
    with_size = build_quote(
        _single(
            OvkObjectPart(category=BuildingCategory.SMAHUS, area_m2=Decimal("145")),
            InspectionType.FORSTAGANG,
        ),
        taxa,
    )
    without_size = build_quote(
        _single(OvkObjectPart(category=BuildingCategory.SMAHUS), InspectionType.FORSTAGANG),
        taxa,
    )
...
def test_smahus_two_units() -> None:
    ...
    quote = build_quote(
        _single(
            OvkObjectPart(category=BuildingCategory.SMAHUS, unit_count=2),
            InspectionType.FORSTAGANG,
        ),
        taxa,
    )
...
def test_smahus_rejects_recurring_inspection() -> None:
    ...
    with pytest.raises(ValueError, match="förstagångsbesiktning"):
        build_quote(
            _single(OvkObjectPart(category=BuildingCategory.SMAHUS), InspectionType.ATERKOMMANDE),
            taxa,
        )
...
def test_hotell_prices_per_room() -> None:
    ...
    quote = build_quote(
        _single(
            OvkObjectPart(
                category=BuildingCategory.HOTELL,
                room_count=120,
                system_type=VentilationSystemType.FTX,
            ),
            InspectionType.ATERKOMMANDE,
        ),
        taxa,
    ...
    assert line.amount == Decimal(120) * taxa.rate(
        BuildingCategory.HOTELL, InspectionType.ATERKOMMANDE
    )
...
def test_hotell_requires_room_count() -> None:
    ...
    with pytest.raises(ValueError, match="room_count is required"):
        build_quote(
            _single(OvkObjectPart(category=BuildingCategory.HOTELL), InspectionType.FORSTAGANG),
            taxa,
        )
    with pytest.raises(ValueError, match="room_count must be >= 1"):
        build_quote(
            _single(
                OvkObjectPart(category=BuildingCategory.HOTELL, room_count=0),
                InspectionType.FORSTAGANG,
            ),
            taxa,
        )
...
def test_lokal_prices_per_area_and_system_type() -> None:
    ...
    ftx = build_quote(
        _single(
            OvkObjectPart(
                category=BuildingCategory.LOKAL,
                system_type=VentilationSystemType.FTX,
                area_m2=Decimal("2400"),
            ),
            InspectionType.ATERKOMMANDE,
        ),
        taxa,
    ...
    s = build_quote(
        _single(
            OvkObjectPart(
                category=BuildingCategory.LOKAL,
                system_type=VentilationSystemType.S,
                area_m2=Decimal("2400"),
            ),
            InspectionType.ATERKOMMANDE,
        ),
...
def test_lokal_requires_area_and_system_type() -> None:
    ...
    with pytest.raises(ValueError, match="area_m2 is required"):
        build_quote(
            _single(
                OvkObjectPart(
                    category=BuildingCategory.LOKAL, system_type=VentilationSystemType.FTX
                ),
                InspectionType.FORSTAGANG,
            ),
            taxa,
    ...
    with pytest.raises(ValueError, match="system_type is required"):
        build_quote(
            _single(
                OvkObjectPart(category=BuildingCategory.LOKAL, area_m2=Decimal("500")),
                InspectionType.FORSTAGANG,
            ),
            taxa,
        )
        ...
        OvkQuoteRequest(
            ...
            parts=(
                ...
                OvkObjectPart(
                    ...
                    system_type=VentilationSystemType.FTX,
                    area_m2=Decimal("650"),
                    label="Restaurang och konferens",
                ),
            ),
        ),
        taxa,
        ...
        build_quote(
            OvkQuoteRequest(
                ...
                parts=(
                    ...
                    OvkObjectPart(category=BuildingCategory.SMAHUS),
                ),
            ),
            taxa,
        )
...
def test_base_fee_always_included_above_minimum() -> None:
    ...
    quote = build_quote(
        _single(
            OvkObjectPart(category=BuildingCategory.FLERBOSTADSHUS, apartment_count=100),
            InspectionType.FORSTAGANG,
        ),
        taxa,
    )
...
def test_recurring_intervals_follow_bfs_2011_16() -> None:
    taxa = load_taxa()
    assert (
        taxa.recurring_interval_years(BuildingCategory.FLERBOSTADSHUS, VentilationSystemType.FTX)
        == 3
    )
    assert (
        taxa.recurring_interval_years(BuildingCategory.FLERBOSTADSHUS, VentilationSystemType.F) == 6
    )
    assert (
        taxa.recurring_interval_years(
            BuildingCategory.LOKAL, VentilationSystemType.F, school_or_care=True
        )
        == 3
    )
...
def test_quote_is_deterministic() -> None:
    taxa = load_taxa()
    request = OvkQuoteRequest(
        inspection_type=InspectionType.FORSTAGANG,
        parts=(
            OvkObjectPart(
                category=BuildingCategory.LOKAL,
                system_type=VentilationSystemType.FTX,
                area_m2=Decimal("3150.5"),
            ),
        ),
    )
...
def test_payload_serialises_money_as_strings() -> None:
    ...
    quote = build_quote(
        _single(
            OvkObjectPart(category=BuildingCategory.FLERBOSTADSHUS, apartment_count=12),
            InspectionType.ATERKOMMANDE,
        ),
        taxa,
    )
...
def test_negative_sizes_are_rejected() -> None:
    ...
    with pytest.raises(ValueError, match="apartment_count must be >= 1"):
        build_quote(
            _single(
                OvkObjectPart(category=BuildingCategory.FLERBOSTADSHUS, apartment_count=0),
                InspectionType.ATERKOMMANDE,
            ),
            taxa,
        )
